...
from transformers import AutoProcessor, LlavaForConditionalGeneration, CLIPProcessor, CLIPModel,CLIPConfig
import transformers
...
def parse_args():
    ...
    parser.add_argument("--img_path", type=str, default="demo.png", help="path to the clean image")
...
def print_gpu_usage(accelerator=None, detailed=False):
    """
    打印当前GPU使用情况，包括进程信息和内存状态
    
    Args:
        accelerator: Accelerator对象（可选）
        detailed: 是否显示详细信息，包括其他进程（可选）
    """
    ...
    for i in range(device_count):
        if i==int(current_device):
            ...
            mem_utilization = (mem_reserved / mem_total) * 100 if mem_total > 0 else 0
            
            # 状态标识
            status_indicators = []
            if i == current_device:
                status_indicators.append("CURRENT")
            if mem_utilization > 90:
                status_indicators.append("HIGH_USAGE")
            if mem_free < 1:  # 小于1GB空闲
                status_indicators.append("LOW_MEMORY")
            
            status_str = f" [{', '.join(status_indicators)}]" if status_indicators else ""
            print(f"GPU {i}: {props.name}{status_str}  Memory: {mem_alloc:.2f}/{mem_reserved:.2f}/{mem_total:.2f} GB (alloc/reserved/total)   Free: {mem_free:.2f} GB | Utilization: {mem_utilization:.1f}%")
...
class WrapAdvModel(torch.nn.Module):
    ...
    def forward(self, 
        ...
        print("computing clip matching loss")
